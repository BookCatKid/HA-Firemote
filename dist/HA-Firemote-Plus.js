...
function handlehdmi(config, inputs = 0) {
  if( inputs > 0 ) {
    for (let i = 1; i <= inputs; i++) {
      ...
      else if (config.device_family == "samsung-tv") {
        appmap.set(configitem, {"button": inputname, "friendlyName": friendlyname, "appName": "HDMI "+i});
      }
      else {
...
function handleTunerInputs(config, tunerInput) {
    ...
    if (config.device_family == "samsung-tv" && (tunerInput == true || tunerInput == 'true')) {
        appmap.set("tuner_input", {"button": "TV Tuner", "friendlyName": "TV Tuner", "appName": "TV"});
        return;
    }
...
class FiremoteCard extends LitElement {
  ...
  static getConfigElement() {
    return document.createElement("firemote-plus-card-editor");
  }
...
//`
    ...
    getState() {
      ...
      if(this._config.android_tv_remote_entity == '' || typeof this._config.android_tv_remote_entity == 'undefined' || ['amazon-fire', 'samsung-tv', 'tivo'].includes(this._config.device_family)) {
    ...
    getOpenAppID() {
      ...
      else if(this._config.device_family == 'samsung-tv') {
        return this.hass.states[this._config.entity].attributes.source;
      }
      else if(this._config.device_family == 'tivo') {
        return this.hass.states[this._config.entity].attributes.app_name || this.hass.states[this._config.entity].attributes.source;
      }
  ...
  buttonDown(clicked) {
    ...
    function getRemoteEntityForFamily(family) {
      const confname = (family+'_remote_entity').replace(/\-/, '_');
      return _config[confname];
    }

    function sendFamilyRemoteCommand(command, opts = {}) {
      const remoteEntity = getRemoteEntityForFamily(deviceFamily);
      if (!remoteEntity) {
        unsupportedButton();
        return false;
      }
      var mappedCommand = command;
      if (deviceFamily == 'samsung-tv') {
        const map = {
          up:'KEY_UP', down:'KEY_DOWN', left:'KEY_LEFT', right:'KEY_RIGHT',
          select:'KEY_ENTER', back:'KEY_RETURN', home:'KEY_HOME', menu:'KEY_MENU',
          input:'KEY_SOURCE', tv:'KEY_TV', info:'KEY_INFO', guide:'KEY_GUIDE',
          rewind:'KEY_REWIND', fast_forward:'KEY_FF', volume_up:'KEY_VOLUP', volume_down:'KEY_VOLDOWN',
          volume_mute:'KEY_MUTE', channel_up:'KEY_CHUP', channel_down:'KEY_CHDOWN',
          color_f0eb53:'KEY_YELLOW', color_f02840:'KEY_RED', color_12d85e:'KEY_GREEN', color_1450e8:'KEY_CYAN',
      ...
      if(typeof mappedCommand === 'undefined') {
        unsupportedButton();
        return false;
      }
      _hass.callService('remote', 'send_command', {
        entity_id: remoteEntity,
        command: mappedCommand,
        num_repeats: opts.num_repeats || 1,
        delay_secs: opts.delay_secs || 0,
        hold_secs: opts.hold_secs || 0
      });
      return true;
    }

    function repeatFamilyRemoteCommand(command, interval = 250) {
      sendFamilyRemoteCommand(command);
    ...
    if(appmap.has(appkey)) {
      ...
      fireEvent(this, 'haptic', 'light');
      if (typeof remoteCommand != 'undefined' && ['apple-tv', 'roku', 'samsung-tv', 'tivo'].includes(deviceFamily)) {
        ...
        switch (deviceFamily) {
          ...
          case 'samsung-tv':
          case 'tivo':
            data['entity_id'] = getRemoteEntityForFamily(deviceFamily);
            break;
        }
    ...
    async function getActionType(that) {
      ...
      if(actionType == 'click' || actionType == 'hold') {
        ...
        if(['samsung-tv', 'tivo'].includes(deviceFamily)) {
          ...
          const clickMap = {
            'up-button':'up','left-button':'left','right-button':'right','down-button':'down','center-button':'select',
            'back-button':'back','home-button':'home','hamburger-button':'menu','settings-button':'menu','options-button':'info',
            'rewind-button':'rewind','fastforward-button':'fast_forward','replay-button':'replay',
            'volume-up-button':'volume_up','volume-down-button':'volume_down','mute-button':'volume_mute',
            'channel-up-button':'channel_up','channel-down-button':'channel_down',
            'num0-button':'num0','num1-button':'num1','num2-button':'num2','num3-button':'num3','num4-button':'num4',
            'num5-button':'num5','num6-button':'num6','num7-button':'num7','num8-button':'num8','num9-button':'num9',
            'red-button':'color_f02840','green-button':'color_12d85e','yellow-button':'color_f0eb53','blue-button':'color_1450e8',
            'input-button':'input','tv-button':'tv','info-button':'info'
          };
          const holdMap = {
            'up-button':'up','left-button':'left','right-button':'right','down-button':'down',
            'volume-up-button':'volume_up','volume-down-button':'volume_down',
            'channel-up-button':'channel_up','channel-down-button':'channel_down',
            'rewind-button':'rewind','fastforward-button':'fast_forward'
          };
          if(actionType == 'click' && typeof clickMap[buttonID] !== 'undefined') {
            sendFamilyRemoteCommand(clickMap[buttonID]);
            return;
          }
          if(actionType == 'hold' && typeof holdMap[buttonID] !== 'undefined') {
            repeatFamilyRemoteCommand(holdMap[buttonID], ['rewind-button','fastforward-button'].includes(buttonID) ? 150 : 250);
            return;
          }
          if(actionType == 'hold') {
            return;
...
customElements.define('firemote-plus-card', FiremoteCard);


// Allow this card to appear in the card chooser menu
window.customCards = window.customCards || [];
window.customCards.push({
  type: "firemote-plus-card",
  name: "Firemote Plus Card",
  preview: true,
  description: "Remote control card for Amazon FireTV, Apple TV, Samsung TV, TiVo, Chromecast, NVIDIA Shield, onn., Roku, and Xiaomi devices"
});
...
class FiremoteCardEditor extends LitElement {
  ...
  getMediaPlayerEntityDropdown(optionValue){
    ...
    if(this._config.device_family == 'apple-tv') {
      ...
      mediaPlayerEntities = this.getMediaPlayerEntitiesByPlatform('samsungtv');
      heading = this.hass.config.language == 'he' || this.hass.config.language == 'fr' ?
        this.translateToUsrLang('Entity') + ' Samsung TV Media Player' : 'Samsung TV Media Player '+ this.translateToUsrLang('Entity');
    }
    else if(this._config.device_family == 'tivo') {
  ...
  getAssociatedRemoteEntityDropdown(optionValue){
    ...
    if(this._config.device_family == 'samsung-tv') {
        var dropdownLabel = this.translateToUsrLang('Associated') + ' Samsung TV Remote ' + this.translateToUsrLang('Entity');
        dropdownLabel = this.hass.config.language == 'he' ?  'ישות משויכת ל-' + 'Samsung TV Remote ' : dropdownLabel;
        if(this._config.samsung_tv_remote_entity == '' || typeof this._config.samsung_tv_remote_entity == 'undefined') {
            blankRemoteEntity = html `<option value="" selected> - - - - </option> `;
        }
        remoteEntities = this.getRemoteEntitiesByPlatform('samsungtv');
        return html`
              ${dropdownLabel}:<br>
              <select name="samsung_tv_remote_entity" id="samsung_tv_remote_entity" style="padding: .6em; font-size: 1em;" .value=${optionValue}
                @focusout=${this.configChanged}
                @change=${this.configChanged} >
                ${blankRemoteEntity}
                ${remoteEntities.map((eid) => {
                  if (eid != this._config.samsung_tv_remote_entity) {
                    return html`<option value="${eid}">${this.hass.states[eid].attributes.friendly_name || eid}</option> `;
                  }
                  else {
                    return html`<option value="${eid}" selected>${this.hass.states[eid].attributes.friendly_name || eid}</option> `;
    ...
    if(this._config.device_family == 'tivo') {
        var dropdownLabel = this.translateToUsrLang('Associated') + ' TiVo Remote ' + this.translateToUsrLang('Entity');
        dropdownLabel = this.hass.config.language == 'he' ?  'ישות משויכת ל-' + 'TiVo Remote ' : dropdownLabel;
        if(this._config.tivo_remote_entity == '' || typeof this._config.tivo_remote_entity == 'undefined') {
            blankRemoteEntity = html `<option value="" selected> - - - - </option> `;
        }
        remoteEntities = this.getRemoteEntitiesByPlatform('tivo');
        remoteEntities = [...remoteEntities, ...this.getRemoteEntitiesByPlatform('pytivo')];
        remoteEntities = [...new Set([...remoteEntities, ...this.getEntitiesByType('remote').filter((eid) => /tivo/i.test(eid))])];
        return html`
              ${dropdownLabel}:<br>
              <select name="tivo_remote_entity" id="tivo_remote_entity" style="padding: .6em; font-size: 1em;" .value=${optionValue}
                @focusout=${this.configChanged}
                @change=${this.configChanged} >
                ${blankRemoteEntity}
                ${remoteEntities.map((eid) => {
                  if (eid != this._config.tivo_remote_entity) {
                    return html`<option value="${eid}">${this.hass.states[eid].attributes.friendly_name || eid}</option> `;
                  }
                  else {
                    return html`<option value="${eid}" selected>${this.hass.states[eid].attributes.friendly_name || eid}</option> `;
  ...
  getCompatibilityModeDropdown(optionValue, deviceFriendlyName){
    if(['apple-tv', 'chromecast', 'homatics', 'nvidia-shield', 'onn', 'roku', 'samsung-tv', 'tivo', 'none'].includes(this._config.device_family)) { return; }
...
customElements.define("firemote-plus-card-editor", FiremoteCardEditor);
